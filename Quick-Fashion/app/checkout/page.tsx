...
export default function CheckoutPage() {
  ...
  const subtotal =
    cart?.items.reduce(
      (sum, item) => sum + Number(item.product.priceInRupees) * item.quantity, 0) || 0;
  ...
  const pay = async () => {
    ...
    if (Object.values(form).some((value) => !value.trim())) {
      return setMessage(
        'Complete every contact and delivery field.'
      );
    }
    ...
    try {
      ...
      if (!checkoutLoaded) {
        throw new Error(
          'Could not load secure payment checkout'
        );
      }
      ...
      if (!response.ok) {
        throw new Error(
          order.error || 'Could not create order'
        );
      }
      ...
      const razorpay = new window.Razorpay({
        ...
        handler: async (payment: Success) => {
          ...
          if (!verificationResponse.ok) {
            setMessage(
              result.error || 'Verification pending'
            );
  ...
  const mockPay = async () => {
    ...
    try {
      const response = await fetch('/api/payments/mock/checkout', {
        ...
        body: JSON.stringify({ deliveryAddress: form }),
      });
      ...
      const inventory = (result.inventory as Array<{
        title: string;
        stockQuantity: number;
      }>)
        .map((product) => `${product.title}: ${product.stockQuantity}`)
        .join(', ');
      setMessage(
        `Mock payment completed. Order ${result.orderId}`,
      );
    } catch (error) {
      setMessage(
      ...
      <div className="checkout-main">
        ...
        <section className="checkout-section">
          ...
          <p className="payment-note">
            Your complete bag is processed as one
            server-verified Razorpay order.
          </p>
